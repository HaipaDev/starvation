...
public class StarvePlayerProcedure {
	public StarvePlayerProcedure() {
		ServerLivingEntityEvents.ALLOW_DAMAGE.register((entity, damageSource, amount) -> {
			if (entity != null) {
				execute(entity.level(), damageSource, entity);
			}
			return true;
		});
	}
	...
	public static void execute(LevelAccessor world, DamageSource damagesource, Entity entity) {
		...
		if ((damagesource).is(DamageTypes.STARVE)) {
			if (world.getDifficulty() == Difficulty.HARD) {
				if ((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) <= healthToDieFromStarvationHard) {
				...
				if ((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) > hardDealDamageTill && hardStarveDmg > 0) {
					if (entity_ instanceof LivingEntity _entity)
						_entity.hurt(new DamageSource(_entity.level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.GENERIC)) {
							@Override
							public Component getLocalizedDeathMessage(LivingEntity _msgEntity) {
						...
						}, (float) Math.min(Math.abs((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) - hardDealDamageTill), hardStarveDmg));
			...
			if (world.getDifficulty() == Difficulty.NORMAL) {
				if ((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) <= healthToDieFromStarvationNormal) {
					if (entity_ instanceof LivingEntity _entity)
						_entity.hurt(new DamageSource(_entity.level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.GENERIC)) {
							@Override
							public Component getLocalizedDeathMessage(LivingEntity _msgEntity) {
				...
				if (normalStarveDmg == 0) {
					if ((entity_ instanceof Player _plr ? _plr.getFoodData().getFoodLevel() : 0) <= 1) {
						if (entity_ instanceof Player _player)
							_player.getFoodData().setFoodLevel(1);
					}
				}
				if ((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) > normalDealDamageTill && normalStarveDmg > 0) {
					if (entity_ instanceof LivingEntity _entity)
						_entity.hurt(new DamageSource(_entity.level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.GENERIC)) {
							@Override
							public Component getLocalizedDeathMessage(LivingEntity _msgEntity) {
						...
						}, (float) Math.min(Math.abs((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) - normalDealDamageTill), normalStarveDmg));
					if ((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) <= 0) {
						entity_ = null;
					}
				} else {
			...
			if (world.getDifficulty() == Difficulty.EASY) {
				...
				if ((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) > easyDealDamageTill && easyStarveDmg > 0) {
					if (entity_ instanceof LivingEntity _entity)
						_entity.hurt(new DamageSource(_entity.level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.GENERIC)) {
							@Override
							public Component getLocalizedDeathMessage(LivingEntity _msgEntity) {
					...
					if (easyDealDamageTill <= 10 && (entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) <= 11) {
						new Object() {
							private int ticks = 0;
			...
			if (world.getDifficulty() == Difficulty.PEACEFUL) {
				...
				if ((entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) > peacefulDealDamageTill && peacefulStarveDmg > 0) {
					if (peacefulDealDamageTill <= 10 && (entity_ instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) <= 11) {
						if (!(entity_ == null)) {
							StarvePlayerBelowProcedure.execute(world, entity);
						}
					} else {
							...
							new Object() {
								...
								public void startDelay(LevelAccessor world) {
									ServerTickEvents.END_SERVER_TICK.register((server) -> {
										this.ticks++;
										if (this.ticks == 5) {
											StarvePlayerHighDmgPeacefulProcedure.execute(world, entity);
											return;
										}
									});
								}
